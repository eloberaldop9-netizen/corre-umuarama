...
export const Jovens1_Oportunidades: React.FC<{ assets: JovensAssets }> = ({ assets }) => {
  ...
  const opWord = (clip?: string) => (
    <div style={{ position: 'absolute', top: 700, left: 0, right: 0, textAlign: 'center', clipPath: clip }}>
      <span style={{ display: 'inline-block', fontFamily: ED.sans, fontWeight: 900, fontSize: 100, lineHeight: 1, color: ED.yellow, whiteSpace: 'nowrap', textShadow: '0 16px 50px rgba(0,0,0,0.6)', ...opStyle }}>
        OPORTUNIDADES
      </span>
    </div>
  );
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: ED.void, overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ transform: `scale(${dolly})` }}>
        ...
        <div style={{ position: 'absolute', top: 120, left: 0, right: 0, textAlign: 'center', ...tear(-1, 2) }}>
        ...
        <div style={{ ...tear(-1), opacity: pIn * (tear(-1).opacity as number) }}>
          <NewsSheet w={560} h={500} seed={5} style={{ top: 300, left: 560, transform: 'rotate(5deg)' }} />
          <NewsSheet w={420} h={320} seed={9} tone="#D9D4CB" style={{ top: 980, left: -40, transform: 'rotate(-4deg)' }} />
        </div>
        <div style={{ position: 'absolute', top: 330, left: -30, ...tear(-1, 1) }}>
          <TornPhoto frame={frame} at={WORD.jovens - 10} file={assets.grupo} w={620} h={400} pos="50% 40%" seed={2} rot={-5} />
        </div>
        <div style={{ position: 'absolute', top: 880, left: 480, ...tear(-1, 1) }}>
          <TornPhoto frame={frame} at={WORD.mercado - 12} file={assets.escritorio} w={620} h={370} pos="55% 60%" seed={6} rot={4} />
        </div>
        ...
        <div style={{ position: 'absolute', top: 400, left: 260, width: 560, ...tear(-1) }}>
          <div style={{ opacity: pIn, filter: `brightness(${pIn}) blur(${20 * (1 - pIn)}px)`, transform: 'rotate(2deg)', position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              <Img src={cut} style={{ display: 'block', width: 560, filter: 'grayscale(1) contrast(1.2) brightness(1.02)' }} />
              <div
                style={{
        ...
        {/* PARA OS JOVENS */}
        <div style={{ position: 'absolute', top: 818, left: 0, right: 0, textAlign: 'center', ...tear(1, 3) }}>
          <span style={{ display: 'inline-block', fontFamily: ED.sans, fontWeight: 900, fontSize: 80, lineHeight: 1, color: ED.yellow, whiteSpace: 'nowrap', textShadow: '0 12px 40px rgba(0,0,0,0.7)', ...edIn(frame, WORD.jovens - 6, { y: 14, trackFrom: -6, trackTo: -2 }) }}>
            PARA OS JOVENS
          </span>
        </div>

        {/* entrarem preparados no / MERCADO DE TRABALHO! */}
        <div style={{ position: 'absolute', top: 1110, left: 0, right: 0, textAlign: 'center', ...tear(1, 5) }}>
          ...
          <div style={{ marginTop: 16, display: 'flex', justifyContent: 'center' }}>
            <div style={{ background: '#FFFFFF', padding: '16px 36px 10px', transform: 'rotate(-2deg)', boxShadow: '0 30px 70px rgba(0,0,0,0.6)', ...edIn(frame, WORD.mercado, { y: 30, blur: 12, trackFrom: 0, trackTo: 0 }), letterSpacing: undefined }}>
              <span style={{ display: 'inline-block', fontFamily: ED.sans, fontWeight: 900, fontSize: 70, lineHeight: 1, color: ED.void, whiteSpace: 'nowrap', ...edIn(frame, WORD.mercado, { y: 0, blur: 0, trackFrom: -6, trackTo: -3 }) }}>
                MERCADO DE TRABALHO!
              </span>
            </div>
          </div>
      ...
      {diving && (
        <AbsoluteFill style={{ transform: `scale(${dolly * (1 + 29 * dive)})`, transformOrigin: '50% 38%', opacity: 1 - ci(dive, [0.6, 1], [0, 1]) }}>
          {opWord()}
        </AbsoluteFill>
      )}
