...
// Cena 3 — O Empreendedorismo | frames locais 0–248
...
// Roxo escuro com grid tecnológico sutil em perspectiva, Z-push contínuo
...
const PILLS = [
  { text: 'com cursos', icon: 'book', at: WORD.cursos - 4, top: 730, x: -170, depth: 1 },
  { text: 'bolsas de capacitação', icon: 'bolsa', at: WORD.bolsas, top: 850, x: 110, depth: 0.9 },
  { text: 'educação financeira', icon: 'moeda', at: WORD.educacao, top: 975, x: -120, depth: 1.05 },
  { text: 'apoio à inovação', icon: 'foguete', at: WORD.apoio, top: 1100, x: 150, depth: 0.92 },
  { text: 'iniciativas', icon: 'alvo', at: WORD.iniciativas - 4, top: 1225, x: -40, depth: 1 },
];
    ...
    <AbsoluteFill style={{ backgroundColor: ED.void, overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ transform: `scale(${pushC * (1 - sp)}) rotate(${-15 * sp}deg)`, filter: `blur(${30 * sp}px)` }}>
        <div style={{ position: 'absolute', top: 190, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ fontFamily: ED.sans, fontWeight: 800, fontSize: 46, color: ED.lilac, ...edIn(frame, Math.max(3, L(WORD.proposta)), { trackFrom: -4, trackTo: 1 }) }}>
            A PROPOSTA TAMBÉM
          </div>
          <div style={{ marginTop: 6, fontFamily: ED.sans, fontWeight: 800, fontSize: 46, color: '#FFFFFF', ...edIn(frame, L(WORD.tambem) + 5, { trackFrom: -4, trackTo: 1 }) }}>
            INCENTIVA O
          </div>
          <div
            style={{
              marginTop: 34, fontFamily: ED.sans, fontWeight: 900, fontSize: 90, lineHeight: 1, color: ED.yellow, whiteSpace: 'nowrap',
              textShadow: '0 20px 60px rgba(0,0,0,0.7)', opacity: hero, filter: `blur(${20 * (1 - hero)}px)`, transform: `scale(${1.12 - 0.12 * hero})`, letterSpacing: -9 + 5 * hero,
            }}
          >
            EMPREENDEDORISMO
          </div>
          <div style={{ fontFamily: ED.sans, fontWeight: 900, fontSize: 170, lineHeight: 1, color: '#FFFFFF', textShadow: '0 20px 60px rgba(0,0,0,0.7)', ...edIn(frame, L(WORD.jovem), { y: 20, trackFrom: -12, trackTo: -6 }) }}>
            JOVEM
          </div>
        </div>
        ...
        {PILLS.map((p, i) => {
          const at = L(p.at);
          const k = ci(frame, [at - 2, at + 22], [0, 1], Easing.out(Easing.cubic));
          const bob = Math.sin((frame + i * 23) * 0.05) * 8;
          const drift = Math.cos((frame + i * 31) * 0.035) * 6;
          const far = p.depth < 1;
          return (
            <div key={p.text} style={{ position: 'absolute', top: p.top, left: 0, right: 0, display: 'flex', justifyContent: 'center' }}>
              <div
                style={{
                  display: 'flex', alignItems: 'center', gap: 20, padding: '18px 38px 18px 22px', borderRadius: 999,
              ...
              >
                <div style={{ width: 70, height: 70, borderRadius: '50%', border: `2px solid rgba(252,227,0,0.5)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <LineIcon name={p.icon} frame={frame} at={at + 2} size={44} color={ED.yellow} stroke={1.8} />
                </div>
