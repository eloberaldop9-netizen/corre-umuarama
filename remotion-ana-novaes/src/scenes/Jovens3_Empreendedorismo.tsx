...
// Cena 3 — O Empreendedorismo | frames locais 0–256
...
const EXIT = 238; // depois de "iniciativas" terminar (597) — tempo de leitura

const PILLS = [
  { text: 'com cursos', icon: 'book', at: WORD.cursos, top: 690, x: -50, depth: 1 },
  { text: 'bolsas de capacitação', icon: 'bolsa', at: WORD.bolsas, top: 802, x: 40, depth: 0.94 },
  { text: 'educação financeira', icon: 'moeda', at: WORD.educacao, top: 914, x: -40, depth: 1 },
  { text: 'apoio à inovação', icon: 'foguete', at: WORD.apoio, top: 1026, x: 50, depth: 0.94 },
  { text: 'iniciativas', icon: 'alvo', at: WORD.iniciativas - 5, top: 1138, x: 0, depth: 1 },
];
    ...
    <AbsoluteFill style={{ backgroundColor: ED.void, overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ transform: `scale(${pushC * (1 - sp)}) rotate(${-15 * sp}deg)`, filter: `blur(${30 * sp}px)` }}>
        <div style={{ position: 'absolute', top: 250, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div style={{ fontFamily: ED.sans, fontWeight: 800, fontSize: 42, color: ED.lilac, ...edIn(frame, Math.max(3, L(WORD.proposta)), { trackFrom: -4, trackTo: 1 }) }}>
            A PROPOSTA TAMBÉM
          </div>
          <div style={{ marginTop: 6, fontFamily: ED.sans, fontWeight: 800, fontSize: 42, color: '#FFFFFF', ...edIn(frame, L(WORD.tambem) + 5, { trackFrom: -4, trackTo: 1 }) }}>
            INCENTIVA O
          </div>
          <div
            style={{
              marginTop: 28, fontFamily: ED.sans, fontWeight: 900, fontSize: 70, lineHeight: 1, color: ED.yellow, whiteSpace: 'nowrap',
          ...
          <div style={{ fontFamily: ED.sans, fontWeight: 900, fontSize: 136, lineHeight: 1, color: '#FFFFFF', textShadow: '0 20px 60px rgba(0,0,0,0.7)', ...edIn(frame, L(WORD.jovem), { y: 20, trackFrom: -10, trackTo: -5 }) }}>
            JOVEM
          </div>
        </div>
        ...
        {PILLS.map((p, i) => {
          ...
          return (
            <div key={p.text} style={{ position: 'absolute', top: p.top, left: 0, right: 0, display: 'flex', justifyContent: 'center' }}>
              <div
                style={{
                  display: 'flex', alignItems: 'center', gap: 18, padding: '14px 34px 14px 18px', borderRadius: 999,
              ...
              >
                <div style={{ width: 62, height: 62, borderRadius: '50%', border: `2px solid rgba(252,227,0,0.5)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <LineIcon name={p.icon} frame={frame} at={at + 2} size={40} color={ED.yellow} stroke={1.8} />
                </div>
                <span style={{ fontFamily: ED.sans, fontWeight: 800, fontSize: 46, letterSpacing: -1, color: '#FFFFFF', whiteSpace: 'nowrap' }}>{p.text}</span>
              </div>
            </div>
          );
        })}
