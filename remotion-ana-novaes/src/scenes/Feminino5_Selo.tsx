...
import { FEMININO_SCENES } from '../feminino-timing';

// Cena 5 — O Selo (vídeo 07) | frames locais 0–140
...
const S = FEMININO_SCENES.c5;

export const Feminino5_Selo: React.FC = () => {
  const frame = useCurrentFrame();
  const zoom = ci(frame, [0, S.duration], [1, 1.03]);
  const fade = ci(frame, [S.duration - 20, S.duration], [1, 0]);
  const bgIn = ci(frame, [0, 20], [0, 1], Easing.inOut(Easing.cubic)); // dissolve sobre a cena 4
